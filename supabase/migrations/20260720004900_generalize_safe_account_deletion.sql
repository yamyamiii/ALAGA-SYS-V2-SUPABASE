-- Extend permanent deletion to dependency-free non-Administrator accounts.
-- Migrations 46-48 remain unchanged. The established staging table and dynamic
-- foreign-key blocker are reused so Resident compensation stays compatible and
-- current or future profile dependencies fail closed.

create or replace function public.admin_account_deletion_eligibility(
  p_actor_id uuid,
  p_profile_ids uuid[]
)
returns table (
...
begin
  ...
  select
    ...
    profile.id <> p_actor_id
      ...
      and not (
        profile.role <> 'resident'::public.app_role
        and (resident.id is not null or registration.id is not null)
      )
      and not (
        profile.role <> 'resident'::public.app_role
        and exists (
      ...
      and public.resident_account_deletion_blocker(
        profile.id,
        case
          when profile.role = 'resident'::public.app_role then resident.id
          else null
        end
      ) is null as eligible,
    case
      when profile.role = 'resident'::public.app_role then 'resident'
      else 'account'
...
begin
  ...
  if found then
    ...
    if target_profile.role <> 'resident'::public.app_role
      and exists (
        select 1
        from public.assistance_notifications as notification
        where notification.recipient_profile_id = target_profile.id
      ) then
      raise exception 'this account has protected dependencies and cannot be permanently deleted'
        using
          errcode = '23503',
          constraint = 'account_delete_protected_dependencies';
    end if;
  ...
  if target_profile.role <> 'resident'::public.app_role
    and (has_resident or has_registration) then
    raise exception 'this account has protected dependencies and cannot be permanently deleted'
      using
        errcode = '23503',
        constraint = 'account_delete_protected_dependencies';
  ...
  if target_profile.role = 'resident'::public.app_role then
    ...
    raise exception 'staff account deletion does not accept a Resident registration version';
  end if;
  ...
  blocker := public.resident_account_deletion_blocker(
    p_target_profile_id,
    case
      when target_profile.role = 'resident'::public.app_role and has_resident
        then resident_record.id
      else null
    end
  );
  if blocker is not null then
    raise exception '%',
      case
        when target_profile.role = 'resident'::public.app_role then
  ...
  perform public.record_user_management_audit(
    ...
    'Administrator prepared an eligible non-Administrator account for permanent deletion',
...
create or replace function public.admin_restore_account_deletion(
  p_actor_id uuid,
  p_target_profile_id uuid,
...
revoke all on function public.admin_account_deletion_eligibility(uuid, uuid[])
  from public, anon, authenticated, service_role;
revoke all on function public.admin_prepare_account_deletion(uuid, uuid, integer)
  from public, anon, authenticated, service_role;
revoke all on function public.admin_restore_account_deletion(
  uuid, uuid, public.account_status, integer
) from public, anon, authenticated, service_role;

grant execute on function public.admin_account_deletion_eligibility(uuid, uuid[])
  to service_role;
grant execute on function public.admin_prepare_account_deletion(uuid, uuid, integer)
  to service_role;
grant execute on function public.admin_restore_account_deletion(
  uuid, uuid, public.account_status, integer
) to service_role;
